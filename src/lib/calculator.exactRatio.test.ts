...
// or exceed 2 dp), so they must reach the engine as the exact rate the user
// entered — the original decimal string plus its entry direction — rather
// than a rounded price. That is the ExactExchangeRate contract the maximizer
// now accepts.
...
describe("Trade Maximizer — Buying preserves the exact entered ratio", () => {
  ...
  // exactly. This exact reciprocal rate cannot survive a 2-decimal
  // price-per-item conversion, so it must be accepted as an exact rate.
  it("C: budget 200 at 13.95 items/currency (279/20) => receive 2790, spend 200", () => {
    const result = expectOk(optimizeBuyTrade("200", { mode: "itemsPerCurrency", value: "13.95" }));
...
describe("Trade Maximizer — Selling preserves the exact entered ratio", () => {
  ...
  it("D: stock 335 at 1.53 items/currency (153/100) => receive 200, sell 306, remaining 29", () => {
    const result = expectOk(optimizeSellTrade("335", { mode: "itemsPerCurrency", value: "1.53" }));
  ...
  it("E: stock 903 at 1.6 items/currency (8/5) => receive 560, sell 896, remaining 7", () => {
    const result = expectOk(optimizeSellTrade("903", { mode: "itemsPerCurrency", value: "1.6" }));
